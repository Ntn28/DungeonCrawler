...
int main(int argc, char **argv)
{
    ...
    while (window_opened)
    {
        ...
        if (receive_len > 0)
        {
            ...
            const int packet_type = ((maze_packet*)server_packet)->packet_type;
            ...
            if(packet_type == WELCOME)
            {
                ...
                send_packet(s, JOIN, player_id, first.x, first.y, order_count_player, &sin);
            }
            else if(packet_type == JOIN)
            {
                send_packet(s, WELCOME, player_id, first.x, first.y, order_count_player, &sin);

            }
            else if(packet_type == MOVEMENT)
            {
                
            }

            else if (receive_len == 16)
            {
        ...
        {
            // Send packet
            send_packet(s, MOVEMENT, player_id, first.x, first.y, order_count_player, &sin);
